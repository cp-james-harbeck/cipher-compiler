...
// Define two regular expressions for identifying data URLs.
const dataUrlRegex = /^data:([^;,]+)(?:;(charset=utf-8|base64))?,([\s\S]*?)\.data$/;
const dataUrlRegexNoSuffix = /^data:([^;,]+)(?:;(charset=utf-8|base64))?,([\s\S]*?)$/;

// This function takes an ID (presumably a URL) and returns the file type.
const _getType = id => {
  // Use the URL module to parse the ID into its components.
  const o = url.parse(id, true);
  let match;
  
  // If the parsed ID is a href (an HTTP(S) URL), and it matches the data URL format...
  if (o.href && (match = o.href.match(dataUrlRegexNoSuffix))) {
    // Extract the MIME type from the data URL.
    let type = match[1] || '';
    // If the type is 'text/javascript' or 'application/octet-stream', change it to 'application/javascript'.
    if (type === 'text/javascript' || type === 'application/octet-stream') {
      type = 'application/javascript';
    }
    
    let extension;
    let match2;
    // Try to match the type to a list of known application types and extract the extension.
    if (match2 = type.match(/^application\/(light|text|rendersettings|spawnpoint|lore|quest|npc|mob|react|group|wind|vircadia)$/)) {
      extension = match2[1];
    // If the type is 'application/javascript', set the extension to 'js'.
    } else if (match2 = type.match(/^application\/(javascript)$/)) {
      extension = 'js';
    // For other types, use the mimeTypes module to determine the extension.
    } else {
      extension = mimeTypes.extension(type);
    }
    // Return the extension (if one was found) or an empty string.
    return extension || '';
  
  // If the parsed ID is a hash and it specifies a type, return that type.
  } else if (o.hash && (match = o.hash.match(/^#type=(.+)$/))) {
    return match[1] || '';
  
  // If the parsed ID is a query string and it specifies a type, return that type.
  } else if (o.query && o.query.type) {
    return o.query.type;
  
  // If the path of the parsed ID has a file extension, return that extension (converted to lowercase).
  } else if (match = o.path.match(/\.([^\.\/]+)$/)) {
    return match[1].toLowerCase() || '';
  
  // If no type could be determined, return an empty string.
  } else {
    return '';
  }
};
